...
(function () {
	...
	var config = {
		...
		strings: {
			...
			subtotal: 'Ukupno: ',
			discount: 'Popust: ',
			shipping: 'ne uključuje dostavu &amp; porez',
			processing: 'Obrada zahteva...'
		},
	...
	if (!PAYPAL.apps.MiniCart) {
		...
		PAYPAL.apps.MiniCart = (function () {
			...
			var _addCSS = function () {
				...
				css.push('#' + name + ' form { position:fixed; float:none; top:-250px; right: 3%; width:265px; z-index:50; margin:60px 0 0 0; padding:50px 10px 0; min-height:170px; background:#fff url(' + config.assetURL + 'images/minicart_sprite.png) no-repeat -125px -60px; border:1px solid rgb(228, 223, 223); border-top:0; font-size:13px; color:#333; text-align:left; -moz-border-radius:0 0 8px 8px; -webkit-border-radius:0 0 8px 8px; border-radius:0 0 8px 8px; -moz-box-shadow:1px 1px 1px rgba(0, 0, 0, 0.1); -webkit-box-shadow:1px 1px 1px rgba(0, 0, 0, 0.1); box-shadow:1px 1px 1px rgba(0, 0, 0, 0.1); } ');
				css.push('#' + name + ' ul { position:relative; overflow-x:hidden; overflow-y:auto; height:100px; margin:0 0 7px; padding:0; list-style-type:none; border-top:1px solid #ccc; border-bottom:1px solid #ccc; } ');
				css.push('#' + name + ' li { position:relative; margin:-1px 0 0; padding:6px 5px 6px 0; } ');
				...
				css.push('#' + name + ' p { margin:0; padding:0 0 0 20px; font-size:13px; font-weight:bold; } ');
				css.push('#' + name + ' p:hover { cursor:pointer; } ');
				css.push('#' + name + ' p input { float:right; text-decoration:none; font-weight:normal; width:260px; font-size: 22px; margin-bottom:8px; margin-top:5px; } ');
			...
			var _buildDOM = function () {
				...
				UI.button.className = 'btn btn-info';
